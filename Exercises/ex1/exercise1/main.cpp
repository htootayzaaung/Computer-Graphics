...
//A struct for left-click button Callback
...
#	endif
	...
	//Putting a mouse motion callback here
	...
	LeftClickData clickData = {0.0, 0.0, false};
    
    // Bind clickData to window
	...
	while( !glfwWindowShouldClose( window ) )
	{
    	...
    	// Drawing code
    	surface.set_pixel_srgb(10, 100, {255, 255, 255});  // Set pixel (10,100) to white
		
		// Additional drawing code could go here...
		Vec2f minCorner = {50, 50};
		Vec2f maxCorner = {150, 150};
		ColorU8_sRGB color = {255, 0, 0};  // Red color

		draw_rectangle_solid(surface, minCorner, maxCorner, color);
		// or for outlined rectangle
		// draw_rectangle_outline(surface, minCorner, maxCorner, color);
...
namespace
{
	...
	void glfw_cb_motion_(GLFWwindow* aWindow, double aMouseXPos, double aMouseYPos)
	{
    	// Retrieve the LeftClickData from the window's user pointer
    	LeftClickData* data = static_cast<LeftClickData*>(glfwGetWindowUserPointer(aWindow));

    	if (!data)
    	{
        	std::fprintf(stderr, "No user data found!\n");
        	return;
    	}

    	// Update the stored mouse position
    	data->mouseX = aMouseXPos;
    	data->mouseY = aMouseYPos;

    	std::printf("Mouse Position - x-coordinate: %.2f, y-coordinate: %.2f\n", aMouseXPos, aMouseYPos);
	}
	...
	void glfw_cb_button_(GLFWwindow* aWindow, int aButton, int aAction, int /*aMod*/)
	{
    	...
    	if (aButton == GLFW_MOUSE_BUTTON_LEFT)
    	{
        	...
        	{
            	std::printf("Left button pressed at x-coordinate: %.2f, y-coordinate: %.2f\n", data->mouseX, data->mouseY);
            	data->isLeftButtonPressed = true;

            	// Perhaps store the click position, or do something with it, e.g.,
            	// surface.set_pixel_srgb(static_cast<int>(data->mouseX), static_cast<int>(data->mouseY), {255, 255, 255});
        	}
        	else if (aAction == GLFW_RELEASE)
        	{
            	std::printf("Left button released at x-coordinate: %.2f, y-coordinate: %.2f\n", data->mouseX, data->mouseY);
